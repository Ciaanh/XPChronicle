# Classic Bar Layer & Frame Positioning Comparison

## Overview
This document provides a detailed comparison of the layer and frame positioning across three XP bar implementations:
- **Classic Bar ** (Current implementation)
- **Classic Bar V1** (Previous implementation)
- **Blizzard XP Bar** (Native implementation reference)

---

## 1. CLASSIC BAR  (Current - Single Frame Architecture)

### Main Frame Structure
```
Frame "ClassicBarTemplate" (frameStrata="LOW", size: config-driven 571x17)
...
## 2. CLASSIC BAR V1 (Container Architecture)

### Container Structure
```
Frame "XPC_ClassicXPBarContainerTemplate" (size: 571x17)
├─ Layers
│  └─ OVERLAY: BarFrameTexture (border atlas)
└─ Frames
   ├─ Bar (inherits XPC_ClassicXPBarTemplate, positioned at BOTTOMLEFT 1,5)
   └─ BelowBarTextContainer
```

### Bar Template Structure
```
Frame "XPC_ClassicXPBarTemplate" (frameStrata="LOW", size: 565x11)
├─ Layers
...
#### **Container Frame** (XPC_ClassicXPBarContainerTemplate)
...
#### **Bar Frame** (XPC_ClassicXPBarTemplate)
...
|----------------|----------------|--------------|----------------|
| **** | FlashFrame (separate) | LOW | Inset (3,-3) matches StatusBar |
...
|----------------|-----------------|---------------|-------------|
| **** | BorderFrame.Texture | MEDIUM | useAtlasSize=true, centered |
...
|----------------|-------------------|---------|
| **** | TOPLEFT (1,0), BOTTOMRIGHT (-1,1) | Prevent bleeding outside border |
...
|----------------|-------------|----------------|
| **** | OverlayFrameTextContainer (MEDIUM, 571x11 centered) | BelowBarTextContainer (MEDIUM, relative to BOTTOMLEFT) |
...
| Frame/Element |  Size | V1 Size | Notes |
|---------------|---------|---------|-------|
| **Main/Container Frame** | 571x17 (config) | 571x17 (explicit) | Same |
| **Bar Frame** | N/A (single frame) | 565x11 (explicit) |  has no separate bar frame |
| **StatusBar** | ~565x11 (anchored) | 565x10 (explicit) |  calculated from insets, V1 explicit |
| **StatusBar Position** | TOPLEFT (3,-3) to BOTTOMRIGHT (-3,3) | BOTTOMLEFT (0,0) in bar frame at (1,5) |  inset, V1 offset in container |
| **Background Texture** | ~563x10 (inset 1px) | 565x10 (fills StatusBar) |  adds safety inset |
| **ExhaustionLevelFillBar** | 0x11 (animated width) | 0x10 (animated width) |  is 1px taller |
| **QuestOverlays** | 0x11 (animated width) | 0x10 (animated width) |  is 1px taller |
...
| **OverlayTextContainer** | 571x11 (explicit) | 565x11 (setAllPoints on bar) |  is 6px wider |
| **BelowTextContainer** | 565x30 (explicit) | Variable (template) | Both use same template |

### Effective Visual Sizes

| Visual Element |  Visible Area | V1 Visible Area | Delta |
|----------------|-----------------|-----------------|-------|
| **Total Frame** | 571x17 | 571x17 | ✅ Same |
| **XP Fill Bar** | ~565x11 | 565x10 |  is 1px taller |
| **Border** | 571x17 | 571x17 | ✅ Same |
| **Rested Overlay** | 0-565 x 11 | 0-565 x 10 |  is 1px taller |
| **Flash Effect** | ~565x11 | 565x11 | ✅ Same |
| **Text Area (on-bar)** | 571x11 | 565x11 |  is 6px wider (centered) |

### Position Offsets

| Element |  Offset from Main | V1 Offset from Container | Calculation |
|---------|---------------------|--------------------------|-------------|
| **StatusBar Top-Left** | (3, -3) | (1, 5) | : 3px right, 3px down; V1: 1px right, 5px up |
| **StatusBar Bottom-Right** | (-3, 3) | (566, 15) | : 3px from edges; V1: explicit container position |
| **Border** | (0, 0) centered | (0, 0) fills container |  and V1 both fill their respective frames |
| **Flash** | (3, -3) matches StatusBar | (0, 0) fills bar frame |  inset, V1 fills |
| **On-Bar Text** | (0, 0) centered | (0, 0) setAllPoints |  centered on main, V1 fills bar |
...
| Element |  Position | V1 Position | Delta |
|---------|-------------|-------------|-------|
| **Main Frame** | Size from config (571x17) | Container: 571x17 explicit | Same effective size |
| **StatusBar** | Anchors: TOPLEFT (3,-3), BOTTOMRIGHT (-3,3) | Size: 565x10, Position: BOTTOMLEFT (0,0) |  uses insets, V1 explicit size |
| **Background Texture** | TOPLEFT (1,0), BOTTOMRIGHT (-1,1) within StatusBar | TOPLEFT, BOTTOMRIGHT (fills StatusBar) |  adds 1px inset |
| **Flash** | FlashFrame at (3,-3) inset, OVERLAY layer | Bar frame layer OVERLAY sub=3 |  separate frame, same coverage |
| **Border** | BorderFrame MEDIUM strata, centered atlas | Container OVERLAY layer, useAtlasSize |  separate frame vs container layer |
| **On-Bar Text** | OverlayFrameTextContainer centered (0,0) | OverlayFrame setAllPoints on bar |  centered, V1 fills bar |
...
**Problem**:  border appeared smaller than V1  
...
### For Performance:
- 's separate FlashFrame is clean but adds one more frame
- V1's layer-based flash is more efficient
- Consider: Move flash back to main frame layers if strata ordering allows

### For Maintainability:
- 's explicit BorderFrame is clearer than V1's container layer
