...
function StatsFrameMixin:OnLoad()
    ...
    if statsFrame.CloseButton then
        statsFrame.CloseButton:SetScript(
            "OnClick",
            function(btn)
                btn:GetParent():Hide()
            end
...
function Stats:RegisterEventHandlers()
    ...
    f.RegisterEvent = f.RegisterEvent or function(_, _)
        end
    f.UnregisterAllEvents = f.UnregisterAllEvents or function(_)
        end
    f.SetScript = f.SetScript or function(_, _, _)
        end
...
function Stats:UpdateLevelStats(statsFrame)
    if not (statsFrame and statsFrame.LeftPage) then
        return
    end
...
function Stats:UpdateSessionStats(statsFrame)
    if not (statsFrame and statsFrame.RightPage) then
        return
    end
